...
$videos = filter_videos($filters);
$years = array_unique(array_filter(array_column($videos, 'year')));
rsort($years);
[$videos, $pagination] = paginate($videos, 12);

render_header('Videos', 'videos');
?>
...
<div class="container-fluid px-3 px-lg-4 py-4">
    <div class="d-flex flex-column flex-md-row align-items-md-end justify-content-between gap-3 mb-4">
        <div>
            ...
            <p class="text-secondary small mb-0"><?= count($videos) ?> videos</p>
        </div>
        <form class="d-flex gap-2" method="get" action="">
            <div class="input-group" style="min-width:240px">
                ...
                <input type="search" name="q" value="<?= e($filters['q']) ?>" class="form-control" placeholder="Search videos…">
            </div>
    ...
    <div class="collapse" id="filterPanel">
        <form method="get" action="" class="card-media p-3 mb-4">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3 d-flex align-items-end"><a href="<?= url('videos.php') ?>" class="btn btn-ghost w-100">Reset</a></div>
            </div>
        </form>
    </div>

    <?php if (empty($videos)): ?>
        <div class="empty-state"><i class="bi bi-play-btn"></i><p>No videos match your filters.</p></div>
    <?php else: ?>
